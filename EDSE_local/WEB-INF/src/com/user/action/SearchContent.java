...
public class SearchContent extends HttpServlet
{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		String hashkey1=hs.getAttribute("hashkey").toString();
		
		//String urank1=UserDAO.userRank(id);
		//String urank=UserDAO.userRankKey(urank1);
		...
		if(st.length==1)
		{
			...
			String hashkey=Hashing.HashingFunction(key, hashkey1);
		...
		else if(st.length<6)
		{
			...
			HashMap<String, Double> hash0 = new HashMap<String, Double>();
			...
			HashMap<String, Double> hashTemp = new HashMap<String, Double>();
			HashMap<String, Double> hashTemp1 = new HashMap<String, Double>();
			HashMap<String, Double> hashTemp2 = new HashMap<String, Double>();
			HashMap<String, Double> hashTemp3 = new HashMap<String, Double>();
			int tcount=0;
			StringBuffer sb = new StringBuffer();
			...
			for(int i=0;i<st.length;i++)
			{
				if(count==0)
				{
					hash0=msrch.searchKey(st[i], hashkey1);
					count++;
				}else if(count==1)
					{
						hash1=msrch.searchKey(st[i], hashkey1);
						count++;
					}else if(count==2)
					{
						hash2=msrch.searchKey(st[i], hashkey1);
						count++;
					}else if(count==3)
					{
						hash3=msrch.searchKey(st[i], hashkey1);
						count++;
					}else if(count==4)
					{
						hash4=msrch.searchKey(st[i], hashkey1);
						count++;
					}
